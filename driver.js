...
async function waitForOneSignalSubId(OneSignal, timeoutMs = 8000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    try {
      const optedIn = !!OneSignal?.User?.PushSubscription?.optedIn;
      const subId = OneSignal?.User?.PushSubscription?.id || "";
      if (optedIn && subId) return { optedIn, subId };
    } catch (_) {}
    await new Promise(r => setTimeout(r, 250));
  }
  return { optedIn: !!OneSignal?.User?.PushSubscription?.optedIn, subId: OneSignal?.User?.PushSubscription?.id || "" };
}
...
async function syncPushToWorker(OneSignal) {
  ...
  if (!optedIn || !subId) {
    // OneSignal peut mettre quelques secondes à fournir l'ID après la permission
    const waited = await waitForOneSignalSubId(OneSignal, 8000);
    optedIn = waited.optedIn;
    subId = waited.subId;
  }
